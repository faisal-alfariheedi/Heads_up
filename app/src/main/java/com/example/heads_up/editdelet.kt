...
class editdelet : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_editdelet)
        init()
...
//        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (apif != null) {
                apif!!.deldat(intent.getIntExtra("id", 0)).enqueue(object : Callback<celep.dat> {
                    override fun onResponse(
                        call: Call<celep.dat>, response: Response<celep.dat>
                    ) {
                        wait(false)
                        inname.setText("")
                        tab1.setText("")
...
//        }
    ...
    fun init() {
        ...
        upbut=findViewById(R.id.addcel)
    ...
    fun updatedb(){
        CoroutineScope(Dispatchers.IO).launch {
            ...
            if (apif != null) {
                apif!!.updatedat(intent.getIntExtra("id", 0), f)
                    ?.enqueue(object : Callback<celep.dat> {
                        override fun onResponse(
                            call: Call<celep.dat>,
                            response: Response<celep.dat>
                        ) {
                            wait(false)
                            inname.setText("")
                            tab1.setText("")
